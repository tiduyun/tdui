// @flow
import { isEmpty, isEqual } from '@tdio/utils'
import { VNode } from 'vue'
import { Component, Prop, Vue } from 'vue-property-decorator'

export type GridItemCallback<T> = (
  i: string,
  w: number,
  h: number,
  // Data
...
export interface PartialPosition { left: number, top: number }
export interface DroppingPosition { left: number, top: number, e: Event }
...
export interface GridResizeEvent { e: Event, node: HTMLElement, size: Size }
...
// export interface Pick<FromType, Properties{ [string]: 0 } > = $Exact <
  ...
  for (let i = itemIndex + 1; i < layout.length; i++) {
    ...
    if (collides(item, otherItem)) {
      resolveCompactionCollision(
        layout,
        otherItem,
        moveToCoord + (item as Kv)[sizeProp],
        axis
      )
    }
  }
...
export function sortLayoutItemsByRowCol (layout: Layout): Layout {
  ...
  return layout.slice(0).sort(function (a: Kv, b: Kv) {
...
export function synchronizeLayoutWithChildren (
  ...
  children.forEach((child, i) => {
    ...
    if (exists) {
      ...
      if (!isProduction && child.data?.attrs?._grid) {
        console.warn(
      ...
      const g = child.data?.attrs?.['data-grid'] || child.data?.attrs?._grid
      ...
      if (g) {
        ...
        layout[i] = cloneLayoutItem({ ...g, i: String(child.key) })
      } else {
...
export function validateLayout (
  ...
  for (let i = 0, len = layout.length; i < len; i++) {
    ...
    for (let j = 0; j < subProps.length; j++) {
      if (typeof (item as Kv)[subProps[j]] !== 'number') {
        throw new Error(
    ...
    if (item.static !== undefined && typeof item.static !== 'boolean') {
      throw new Error(
        `GridLayout: ${contextName}[ ${i} ].static must be a boolean!`
      )
    }
  }
}
